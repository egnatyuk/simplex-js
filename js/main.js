...
//Рассчет по текстовому полю
$('div.subsubmit ').live('click', function(){		
	$('#result').html(' '); // Очищаем поле результатов
	$('.manual').slideUp(200);
	...
	//Начинаем заполнять наш массив	
	//Записываем в массив все ограничения
	for (var i = 0; i < massmatrix.length-1; i++) {
		if (massmatrix[i][0] == ">" || massmatrix[i][0] == "="){
			var num_in_mass = massmatrix[i].replace (">","").replace ("=","").split(";")
			for (var j = 0; j < num_in_mass.length; j++) {
				num_in_mass[j] *= -1
			}
		}
		else
		{				
			var num_in_mass = massmatrix[i].replace ("<","").split(";")					
		}
		matrix[i] = num_in_mass
	}

	//Записываем в массив целевую функцию
	if (massmatrix[massmatrix.length-1].split("min").length >1  ){
		var num_in_mass = massmatrix[massmatrix.length-1].replace ("min","").split(";")
	}
	else{				
		var num_in_mass = massmatrix[massmatrix.length-1].replace ("max","").split(";")	
		for (var i = 0; i < num_in_mass.length; i++) {
			num_in_mass[i] *= -1
		}
	}
	num_in_mass.push("0")
	matrix[massmatrix.length-1] = num_in_mass
